...
static int single_client_server_chain_test(env_t env, int fastpath, int prio_diff, seL4_Word threshold)
{
    ...
    /* Get the cspacepath for the endpoint */
    if (threshold!=0) {
        cspacepath_t path;
        vka_cspace_make_path(&env->vka, first_endpoint, &path);
        seL4_CNode_Endpoint_SetThreshold(path.root, path.capPtr, path.capDepth, threshold);
    }
    ...
    for (int i = 0; i < num_proxies; i++) {
        ...
        if (threshold!=0) {
            ...
            seL4_CNode_Endpoint_SetThreshold(path.root, path.capPtr, path.capDepth, threshold - (i+1)*20*US_IN_MS);
        }
...
{
    return single_client_server_chain_test(env, 0, 0, 0);
}
...
{
    return single_client_server_chain_test(env, 0, 1, 0);
}
...
{
    return single_client_server_chain_test(env, 1, 1, 0);
}
...
{
    return single_client_server_chain_test(env, 1, 0, 0);
}
...
// Test successful pass on thresholded endpoint
static int test_client_server_threshold(env_t env)
{
    helper_thread_t client, server;
    int client_prio = 10;
    int server_prio = 15;
...
// Test rejection on thresholded endpoint with max budget < threshold


DEFINE_TEST(IPC0029, "Cross core sched donation", test_sched_donation_cross_core,
            config_set(CONFIG_KERNEL_MCS) &&config_set(CONFIG_MAX_NUM_NODES) &&CONFIG_MAX_NUM_NODES > 1);
...
{
    return single_client_server_chain_test(env, 1, 0, 190*US_IN_MS);
}
...
static void threshold_defer_server_fn(seL4_CPtr receive_endpoint, seL4_CPtr reply, seL4_CPtr sched_context) {
    ...
    // seL4_Time consumed = 0;
    // while(consumed < 500 * US_IN_MS) {
    //     /* Threshold is 700, so consume up to 500 */

    //     seL4_SchedContext_Consumed_t consumed_budget = seL4_SchedContext_Consumed(sched_context);
    //     consumed += consumed_budget.consumed;
    //     printf("Server consumed %lu\n",consumed);
    // }
    volatile int i =0;
    while (i<500000) {
        i++;
...
int test_single_client_threshold_defer(env_t env) {
    ...
    cspacepath_t path;
    vka_cspace_make_path(&env->vka, endpoint, &path);
    error = seL4_CNode_Endpoint_SetThreshold(path.root, path.capPtr, path.capDepth, 100 * US_IN_MS);
    test_eq(error, seL4_NoError);



    error = api_sched_ctrl_configure(simple_get_sched_ctrl(&env->simple, 0), client.thread.sched_context.cptr,
                                200 * US_IN_MS, 1 * US_IN_S, 5, 0);
...
static void budget_limit_client(seL4_CPtr call_endpoint, seL4_CPtr sched_context, seL4_CPtr final_endpoint)
{
...
static void budget_limit_server(seL4_CPtr receive_endpoint, seL4_CPtr reply) {
    ZF_LOGD("Server call");
...
int budget_limit_test(env_t env) {
    helper_thread_t client;
    helper_thread_t server;
    int client_prio = 10;
    ...
    create_helper_thread(env, &server);
    ZF_LOGD("Start server");
    start_helper(env, &server, (helper_fn_t) budget_limit_server, endpoint, server.thread.reply.cptr,
                  client.thread.sched_context.cptr, 0);
    ...
    start_helper(env, &client, (helper_fn_t) budget_limit_client,
                    endpoint, client.thread.sched_context.cptr, timeout_endpoint, 0);
